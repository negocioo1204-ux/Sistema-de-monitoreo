...
for (const file of toolFiles) {
	...
	if (!existsSync(join(toolTestsDir, testFile))) {
		missingToolTests.push({
			src: `src/tools/${file}`,
			expected: `tests/tools/${testFile}`,
		});
	}
}
...
if (missingToolTests.length > 0) {
	...
	console.log(
		`✅ All ${toolFiles.length} tool files have a matching test file (tests/tools/).`,
	);
}
...
for (const file of clientFiles) {
	...
	if (!existsSync(join(clientTestsDir, testFile))) {
		missingClientTests.push({
			src: `src/omadaClient/${file}`,
			expected: `tests/omadaClient/${testFile}`,
		});
	}
}
...
if (missingClientTests.length > 0) {
	...
	console.log(
		`✅ All ${clientFiles.length} omadaClient files have a matching test file (tests/omadaClient/).`,
	);
}
