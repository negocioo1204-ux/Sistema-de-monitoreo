...
if (!existsSync(envPath)) {
	logger.error(
		"Missing .env file at the repository root. Create one before running the MCP Inspector.",
	);
	process.exit(1);
}
...
if (config.useHttp) {
	...
	logger.info(
		`Starting MCP Inspector for streamable-http transport at ${serverUrl}`,
	);
	logger.info(
		"Note: This assumes the HTTP server is already running. Start it first with MCP_SERVER_USE_HTTP=true.",
	);
	...
	if (mode !== undefined && !validModes.includes(mode)) {
		logger.warn(
			`Invalid mode '${mode}'. Using 'dev' instead. Valid modes: ${validModes.join(", ")}`,
		);
	}
	const actualMode =
		mode !== undefined && validModes.includes(mode) ? mode : "dev";

	// Force HTTP mode off for stdio inspector
	process.env.MCP_SERVER_USE_HTTP = "false";

	const serverCommand =
		actualMode === "dev" ? ["tsx", "src/index.ts"] : ["node", "dist/index.js"];
