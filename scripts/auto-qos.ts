...
import {
	OmadaClient,
	type OmadaClientOptions,
} from "../src/omadaClient/index.js";
...
const LOG_FILE =
	process.env.AUTO_QOS_LOG_FILE || path.join(repoRoot, "auto-qos.log");
...
async function collect(client: OmadaClient, siteId: string) {
	...
	try {
		const end = nowUnix();
		const start = end - 300;
		results.retryAndDroppedRate = await client.getRetryAndDroppedRate(
			siteId,
			start,
			end,
		);
	...
	try {
		results.bandwidthControlRules =
			await client.listBandwidthControlRules(siteId);
	} catch (e) {
...
type TrafficClass =
	...
	| "bulk"
	| "background";
const TRAFFIC_CLASSES: TrafficClass[] = [
	"critical",
	"interactive",
	"streaming",
	"bulk",
	...
	predict(features: number[]): TrafficClass {
		...
		for (const cls of TRAFFIC_CLASSES) {
			...
			for (let i = 0; i < this.nFeatures; i++) {
				...
				logProb +=
					-0.5 * Math.log(2 * Math.PI * varI) - (diff * diff) / (2 * varI);
			}
...
function extractTrafficFeatures(
	app: { name: string; rx: number; tx: number },
	allApps: Array<{ name: string; rx: number; tx: number }>,
): number[] {
...
function analyze(state: Record<string, unknown>): AnalysisResult {
	...
	if (retryDetector.isAnomaly(rr))
		result.anomalyFlags.push(`retry-spike:${rr.toFixed(1)}%`);
	if (droppedDetector.isAnomaly(dr))
		result.anomalyFlags.push(`dropped-spike:${dr.toFixed(1)}%`);
	if (ispDetector.isAnomaly(usage))
		result.anomalyFlags.push(`isp-spike:${usage.toFixed(1)}%`);

	// --- 3. Predict congestion probability with logistic regression ---
	const clients = (state.mostActiveClients ?? []) as Array<
		Record<string, unknown>
	>;
	const activeClientCount = clients.length;
	const avgBandwidth =
		activeClientCount > 0
			? clients.reduce((s, c) => s + Number(c.totalTraffic ?? 0), 0) /
				activeClientCount /
				1_048_576
			: 0;
	const congestionFeatures = [
		...
		rr / 100,
		dr / 100,
		Math.min(activeClientCount / 50, 1),
		Math.min(avgBandwidth / 1000, 1),
	];
	result.congestionProbability =
		congestionPredictor.predict(congestionFeatures);
	...
	if (td) {
		const apps = (td.applications ?? td.protocols ?? td.list ?? []) as Array<
			Record<string, unknown>
		>;
		...
		const classified = parsed.map((a) => {
			...
			return {
				...a,
				trafficClass,
				priority: trafficClassifier.getClassPriority(trafficClass),
			};
		});
		...
		const lowPriorityTraffic = classified
			.filter((a) => a.priority <= 1)
			.reduce((s, a) => s + a.rx + a.tx, 0);
		...
		if (result.congested) {
			for (const a of classified.slice(0, 5)) {
				if (a.trafficClass === "bulk" || a.trafficClass === "background") {
					trafficClassifier.partialFit(
						extractTrafficFeatures(a, classified),
						a.trafficClass,
					);
				}
			}
	...
	if (result.actionRequired) {
		log("ML analyze", {
			...
			topTraffic: result.topProtocols
				.slice(0, 3)
				.map((t) => `${t.name}(${t.trafficClass})`),
		});
	}
...
function trainFromObservation(
	prevState: Record<string, unknown>,
	prevResult: AnalysisResult,
) {
	const rdr = prevState.retryAndDroppedRate as
		| Record<string, unknown>
		| undefined;
...
async function act(
	...
	if (
		analysis.actions.includes("rate-limit-hogs") &&
		analysis.bandwidthHogs.length > 0
	) {
		for (const hog of analysis.bandwidthHogs) {
			...
			try {
				await client.setClientRateLimit(
					hog.mac,
				...
				log("act: rate-limited client", {
					name: hog.name,
					mac: hog.mac,
					down: BANDWIDTH_LIMIT_DOWN,
					up: BANDWIDTH_LIMIT_UP,
				});
			} catch (e) {
				log("act: setClientRateLimit failed", {
					mac: hog.mac,
					error: String(e),
				});
			}
	...
	if (
		analysis.actions.includes("throttle-non-critical") &&
		analysis.topProtocols.length > 0
	) {
		const existingRules = (state.bandwidthControlRules ?? []) as Array<
			Record<string, unknown>
		>;
		const lowPriorityApps = analysis.topProtocols
		...
		if (existingRules.length < MAX_RULES) {
			for (const app of lowPriorityApps) {
				try {
					...
					log("act: createBandwidthCtrlRule failed", {
						app: app.name,
						error: String(e),
					});
				}
...
async function removeAllTempRules(
	client: OmadaClient,
	siteId: string,
	state: Record<string, unknown>,
) {
	const rules = (state.bandwidthControlRules ?? []) as Array<
		Record<string, unknown>
	>;
	for (const rule of rules) {
		...
		if (desc.includes("auto-qos") || name.startsWith("auto-qos")) {
			try {
				...
				log("act: deleteBandwidthCtrlRule failed", {
					id: rule.id,
					error: String(e),
				});
			}
...
async function main() {
	...
	log("auto-qos starting", {
		...
		classifiers:
			"GaussianNB(6-features) + LogisticRegression(5-features) + AnomalyDetector(Welford)",
