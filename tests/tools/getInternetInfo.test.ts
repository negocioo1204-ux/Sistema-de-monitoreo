...
describe('tools/getInternetInfo', () => {
    ...
    describe('registerGetInternetInfoTool', () => {
        it('should register the getInternetInfo tool with correct schema', () => {
        ...
        it('should execute successfully with no args', async () => {
            const mockData = { wan: { connected: true } };
            (mockClient.getInternetInfo as ReturnType<typeof vi.fn>).mockResolvedValue(mockData);
            registerGetInternetInfoTool(mockServer, mockClient);
        ...
        it('should execute successfully with siteId', async () => {
            const mockData = { wan: { connected: false } };
            (mockClient.getInternetInfo as ReturnType<typeof vi.fn>).mockResolvedValue(mockData);
            registerGetInternetInfoTool(mockServer, mockClient);
            const result = await toolHandler({ siteId: 'test-site' }, { sessionId: 'test-session' });
            expect(mockClient.getInternetInfo).toHaveBeenCalledWith('test-site', undefined);
            expect(result).toEqual({
                content: [{ type: 'text', text: JSON.stringify(mockData, null, 2) }],
            });
        });
        ...
        it('should handle errors', async () => {
            ...
            (mockClient.getInternetInfo as ReturnType<typeof vi.fn>).mockRejectedValue(error);
            registerGetInternetInfoTool(mockServer, mockClient);
            await expect(toolHandler({}, { sessionId: 'test-session' })).rejects.toThrow('API error');
            expect(loggerModule.logger.error).toHaveBeenCalledWith('Tool failed', {
                tool: 'getInternetInfo',
